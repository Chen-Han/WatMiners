...
app.config(["$stateProvider","$urlRouterProvider",function(n,e){e.otherwise("/"),n.state("home",{url:"/",controller:"IntroCtrl",templateUrl:"template/intro.html"}).state("ranking-input",{url:"/ranking-input",controller:"InputCtrl",templateUrl:"template/ranking-input.html"}).state("about",{url:"/about",templateUrl:"template/about.html"}).state("ranking-display",{url:"/ranking-display",controller:"DisplayCtrl",templateUrl:"template/ranking-display.html"})}]),app.service("$service",["$http","$firebaseAuth","$firebase","$timeout",function(n,e,a){var t=this
this.firebaseURL="https://watminers.firebaseio.com",this.authObj=null,this.refRanking=null,this.rankings=null,this.refJob=null,this.syncRanking=null,this.authData=null,this.login=function(n,a){t.refRanking=new Firebase(t.firebaseURL),t.authObj=e(t.refRanking),t.authObj.$authWithOAuthPopup("facebook").then(function(e){t.authData=e,console.log("Logged in as:",e),Firebase.goOffline(),n&&n()})["catch"](function(n){console.error("Authentication failed:",n),a&&a("login failed, try referesh the page")})},this.didUserRank=function(n,e){Firebase.goOnline()
...
for(var e=0;e<t.rankings.length;e++)!function(){var i=a(new Firebase(t.firebaseURL+"/jobs/"+t.rankings[e].jobID)).$asArray()
i.$loaded().then(function(){n&&n(i)})["catch"](function(n){console.log(n)})}()}t.rankings?i():t.didUserRank(i,e)}}]),app.controller("InputCtrl",["$scope","$rootScope","$service","$state","$stateParams","$firebase",function(n,e,a,t){function i(){t.go("ranking-display")}function o(){return{jobID:"",ranking:"Ranked",preference:0}}if(null===a.authData)t.go("home")
...
o.login(n,e)}}]).controller("DisplayCtrl",["$scope","$rootScope","$service","$state","$stateParams",function(n,e,a,t){a.rankings&&a.authData?(n.rankings=[],a.getRankings(function(e){console.log(e)
for(var a=0;a<e.length;a++)n.rankings.push(e[a])},function(n){console.log(n),t.go("home")})):t.go("home")}])
